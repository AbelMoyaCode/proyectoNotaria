...
/**
 * Datos de trámites para insertar en la base de datos
 */
const tramites = [
    // PODERES
...
/**
 * Función para insertar los trámites en la base de datos
 */
async function seedTramites() {
    try {
        console.log('🌱 Iniciando inserción de trámites...');

        for (const tramite of tramites) {
            const query = `
                INSERT INTO tramites (codigo, nombre, descripcion, requisitos, precio, duracion_estimada, categoria)
        ...
        console.log(`✅ ${tramites.length} trámites insertados/actualizados correctamente`);

        // Verificar total
        const result = await pool.query('SELECT COUNT(*) as total FROM tramites');
        console.log(`📊 Total de trámites en la base de datos: ${result.rows[0].total}`);

    } catch (error) {
        console.error('❌ Error al insertar trámites:', error);
        throw error;
    }
}

module.exports = { seedTramites, tramites };
